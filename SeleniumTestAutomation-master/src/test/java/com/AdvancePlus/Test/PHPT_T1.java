...
public class PHPT_T1 {
	...
	public void test(String URL, String tester) throws Exception {
		try {
			...
			logger.step("Click on the accounts tab");
			homepage.clickAccountsOnLeftRail();

			logger.step("Click on admins");
			homepage.clickAdminsOnLeftRail();

			logger.step("Press on add new account");
			adminPage.clickAddBTN();

			logger.step("Enter fist name");
			adminPage.addNewAccountName("Jack");

			logger.step("Enter new email");
			adminPage.addNewAccountEmail(newEmail);

			logger.step("Enter new password");
			adminPage.addNewAccountPassword(newPassword);

			logger.step("Enter data");
			adminPage.addNewAccountData();

			logger.step("Enter country");
			adminPage.selectCountry("EGYPT");

			logger.step("Click save BTN");
